...
function FormularioEditarVuelo() {
    ...
    useEffect(() => {
        ...
        if (id) {
            fetch(`/api/vuelos/${id}`)
                .then(res => res.json())
                .then(data => {
                    setCodVuelo(data.codvuelo);
                    setCodDestino(data.coddestino);
                    setCodAerolinea(data.codaerolinea);
                    setSalaAbordaje(data.salaabordaje);
                    // Assuming horasalida and horallegada are ISO strings
                    setHoraSalida(data.horasalida.substring(0, 16)); 
                    setHoraLlegada(data.horallegada.substring(0, 16));
                });
        }
    }, [id]);
        ...
        <div className="min-h-screen bg-gray-100">
            <header className="bg-white shadow-lg">
                <div className="container mx-auto px-4 py-6 flex justify-between items-center">
                    <h1 className="text-2xl font-bold text-gray-800">Editar Vuelo</h1>
...
export default FormularioEditarVuelo;
